...
- **Faza 1 — MuJoCo (`unitree_mujoco`)**: oficjalny model G1 od Unitree, natywne `unitree_hg/LowCmd, LowState`, ROS2 bridge w pakiecie. Bipedalne sterowanie dostępne natywnie (nie wymaga mocka chodu). Tu lecą: nav2, slam_toolbox, AMCL, AprilTag visual servo, dock w trybie APRILTAG i LIDAR_LINE, pełen mission BT. **Większość pracy fazowej projektu odbywa się tutaj.** Hardware: dowolny x86_64/aarch64 Linux, GPU opcjonalne (renderowanie kamery przyspiesza, fizyka MuJoCo jest CPU-only).
