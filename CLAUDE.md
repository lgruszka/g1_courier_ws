...
- **1.3 — walking controller** (`/cmd_vel` → motor commands na nogi): conditio sine qua non dla 1.4+. Próbowane ścieżki α i β **zawiodły dla naszego use case** i ich kod został usunięty żeby nie mylić — Faza 1.3 będzie czystym refaktorem od zera kiedy będzie czas:
  ...
  - β2) **Custom RL retraining** (legged_gym + arm-force domain randomization na cloud GPU): realne, dni-tygodnie + koszt cloud GPU. **Najsensowniejsza droga** kiedy 1.3 ruszy.
  - **Obecny workaround dla 1.0..1.2**: pelvis weld pin w mac scene.xml (`<equality><weld>`) plus sim-only `kinematic_mode` w arm action serverach. Robot stoi jak posąg, ramiona z prawdziwą fizyką. Patch w `PELVIS_WELD_PATCH.md`. Wzorowane na `g1_logistics_demo`. Sim-only — usuwa się gdy 1.3 da prawdziwego chodu.
