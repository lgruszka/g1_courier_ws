...
| AMCL + nav2 real `navigate_to_pose` (Faza 1.5) | ✅ pełne; `phase1_full.launch.py` z nav2 stack via jazzy `bringup_launch.py` (AMCL OmniMotionModel + set_initial_pose, NavfnPlanner A\* tolerance 0.25, RotationShim → RegulatedPurePursuit, footprint 0.18×0.20 + inflation 0.40). `nav2_navigate_proxy` adapter dla mission BT contract. cmd_vel_arbiter merge'uje `/cmd_vel_nav` (jazzy remapping) + dock + retreat. Mapa wyczyszczona z 178 szumowych pikseli (connected-component, size<5). `_phase_transfer` z 2 nav goali → 1 (transit_via usunięty, A\* sam routuje). table_a `dock_mode: lidar_line` (carry occluding head_cam też na return leg). Patrz `docs/phases/phase_1_5.md` |
| Real cardboard parcel (Faza 1.5+) | 🔄 in progress — brief 0006 wysłany; mid-pick weld constraint przy stage P3, paczka 0.20×0.15×0.12 m + AprilTag id=10 na top face (hook na v2). Linux side: `/arm/stage_event` topic + `rt/parcel_state` publisher w pick/place_action_server. Friction grasp via finger control (replikacja `g1_logistics_demo`) odłożone do v2 — wymagałoby investigation czy G1 model ma palce, extension `ARM_JOINTS` o finger motor IDs, nowych keyframes z palcami, friction tuningu. Weld zostanie jako fallback `use_friction_grasp: bool` flag gdy v2. |
| Pełen mission BT cycle z real ruchem A↔B (Faza 1.6) | ❌ wymaga walidacji multi-cycle (`max_cycles > 1`) plus stress test |
...
- **1.5 — AMCL + nav2 + real `navigate_to_pose`** (zrobione): `phase1_full.launch.py` z nav2 stack via jazzy `bringup_launch.py`. AMCL OmniMotionModel + set_initial_pose; NavfnPlanner A\*; RotationShim → RegulatedPurePursuit; footprint 0.18×0.20 + inflation 0.40. `nav2_navigate_proxy` translatuje `/courier/navigate_to_pose` (mission BT contract) → `/navigate_to_pose` (nav2). cmd_vel_arbiter merge'uje `/cmd_vel_nav` (jazzy remapping) + dock + retreat → `/cmd_vel`. Mission cycle 4-fazowy (pickup_a + transfer_b + pickup_b + transfer_a) działa end-to-end. table_a `dock_mode: lidar_line` (carry occluding head_cam też na return leg). Mapa `~/maps/lab.pgm` wyczyszczona z 178 szumowych pikseli (connected-component, size<5). Patrz `docs/phases/phase_1_5.md`.
...
- **nav2 controller_server "Passing new path to controller" ale robot stoi** — w jazzy `nav2_bringup/launch/navigation_launch.py` (linie 137, 181, 215) wszystkie nav nodes mają `remappings=[('cmd_vel', 'cmd_vel_nav')]`. Czyli nav2 publikuje na `/cmd_vel_nav`, **nie** `/cmd_vel`. Próba dać arbiterowi `enable_publish: False` plus liczyć że nav2 wstrzeli na `/cmd_vel` bezpośrednio — fail, `/cmd_vel_nav` ląduje w czarnej dziurze. **Fix**: arbiter w trybie merging (`enable_publish: True`, default), subskrybuje `/cmd_vel_nav` + dock + retreat, publikuje `/cmd_vel`. Diagnostyka: `ros2 topic hz /cmd_vel` 0 Hz przy `/cmd_vel_nav` z danymi.
- **nav2 lifecycle aborti — `collision_monitor`/`docking_server`** — jazzy `bringup_launch.py` zawsze odpala oba węzły, brak toggle. Bez stub config: `collision_monitor: observation_sources not initialized`, `docking_server: Charging dock plugins not given!`. Plus pusta lista `[]` lub dict `{}` w YAML rozbija parser z `Expected float/int/str/bool/bytes, got tuple ()`. **Fix**: stub config z jednym disabled item odpowiedniego typu (`polygons: ['FootprintApproach']` z `enabled: false`, `dock_plugins: ['simple_charging_dock']` z fake'owym configiem). Patrz `nav2_params.yaml` linie 188-274.
- **NavfnPlanner `Failed to create plan with tolerance of: X`** — dwa różne przyczyny: (a) `tolerance > distance` (np. 0.50 > 0.50m nav) → planner zwraca empty path, controller dostaje pustą trasę, robot stoi. Fix: `tolerance: 0.25` (~połowa najkrótszego nav distance). (b) Start lub goal w obrębie inflated zone od szumowych pikseli w mapie. Real obstacles tworzą connected components o setkach pikseli, szum to izolowane 1-3 pikseli. **Fix mapy**: connected-component filter `from scipy.ndimage import label`, ustaw na free wszystkie components z `size < 5`. W naszym lab.pgm usunęło 178 pikseli, biurka i ściany pozostały intact. Plus użyj `use_astar: true` dla A\* — Dijkstra zawodzi w wąskich korytarzach.
- **Pure-pursuit oscyluje przy in-place yaw** — `RegulatedPurePursuitController` z `lookahead_dist: 0.6` nie robi czystego in-place rotation, robi małe loopy `lookahead × velocity`. Gdy robot musi zmienić yaw ~180° (po retreat) plus `regulated_linear_scaling_min_radius` spowalnia w ciasnym łuku — robot dryptuje przez kilkadziesiąt sekund. **Fix**: owinąć w `nav2_rotation_shim_controller::RotationShimController` z `rotate_to_goal_heading: true`. Plus znane ograniczenie: pozostaje resztkowy mikro-oscylacja na samym końcu — nie blokuje funkcjonalności (dock i tak rozpoznaje krawędź biurka).
- **Mission BT pętli się nieskończenie po dock_to_table_a timeout** — `_phase_transfer(table_a)` ostatnia faza cycle ma robota carryującego paczkę z B. Head_cam zasłonięty paczką, APRILTAG aligner kręci się szukając tag36h11 id=5, timeout 30 s, Sequence z `memory=True` restartuje cycle od początku → nieskończony nav→dock→timeout. **Fix**: `table_a.dock_mode: lidar_line` w `waypoints.yaml`. RANSAC 360° lidar pelvis-mounted działa parcel-independent. Pierwsza wizyta przy table_a (no carry) też działa. To ten sam problem który mamy z table_b (komentarz w waypoints.yaml już o tym wspominał) — symetryczny dla return leg.
