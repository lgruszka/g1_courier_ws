...
class LidarLineAligner:
    """TODO: line fit + align perpendicular to table edge in 2D scan.

    Algorithm sketch (to implement):
      1. Filter scan to a forward window (e.g. +-30 deg, 0.3..2.0 m).
      2. RANSAC fit a 2D line to the front cluster.
      3. The line's normal in robot frame is the desired heading delta.
      4. The perpendicular distance to the line minus desired_offset is dx.
      5. dy is harder without a feature in the line - keep zero or use gap detection.
    """

    def __init__(self, *, target_distance_m: float, kp_xy: float, kp_yaw: float,
                 max_vx: float, max_vyaw: float) -> None:
    ...
    def step(self, scan: LaserScan) -> tuple[Twist, AlignError]:
        # TODO: implement line fit + control.
...
class DockActionServer(Node):
    ...
    def _run_lidar(self, goal_handle, _request, result, _deadline) -> None:
        # TODO: implement RANSAC line fit + iterative alignment using
        # LidarLineAligner. Mirror the polling loop from _run_apriltag.
        goal_handle.abort()
        result.success = False
        result.message = 'LIDAR_LINE not yet implemented (TODO)'
